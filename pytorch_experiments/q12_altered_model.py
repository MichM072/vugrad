...
class NeuralNetAlt(torch.nn.Module):
    """
    ...
    def __init__(self, lr=0.001, momentum=0.9, model_name=None, variant=0):
        super().__init__()
        if variant in [0, 3]:
            ...
            self.fc2 = nn.Linear(120, 84)
            self.fc3 = nn.Linear(84, 10)
        # Changed SGD to Adam as that is the industry standard.
        self.optimizer = optim.Adam(self.parameters(), lr=lr) if variant in [0, 1] else optim.SGD(self.parameters(), lr=lr, momentum=momentum)
        self.criterion = nn.CrossEntropyLoss()
        self.model_name = model_name if model_name is not None else 'cifar_net'
        self.variant = variant

    def forward(self, x):
        # Adjusted for leaky relu and batch norm
        if self.variant == 2:
...
def objective(trial):
    ...
    net = NeuralNetAlt(lr, momentum, variant=variant)
...
def final_evaluation(param_dict):
    ...
    model_name = f'cifar_net_{global_variant}_{param_dict.values()}'
    net = NeuralNetAlt(param_dict['lr'], param_dict['momentum'], model_name=model_name)
...
if __name__ == '__main__':
    ...
    for global_variant in range(4):
        ...
        score_per_variant.append(final_score)
        print(f'Final accuracy for variant{global_variant}: {final_score:.2f}')

    print(f'Accuracy per variant: {score_per_variant}')
