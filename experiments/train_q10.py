...
class MLP_Relu(vg.Module):
    ...
    def __init__(self, input_size, output_size, hidden_mult=4, hidden2_mult=0.5):
        """
        :param input_size:
        :param output_size:
...
for i, mlp in enumerate([mlp_relu]):
...
ax.plot(x, val_accs, label="Relu")
ax.set_xlabel('epoch')
ax.set_xticks(x)
ax.set_ylabel('accuracy')
ax.legend()
print(f"Relu: {val_accs[-1]:.4}")
plt.savefig('q10_results.png')
plt.show()